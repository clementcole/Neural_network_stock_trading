...
void trainCells(Cell &neuron, std::vector<float> target_prices, string &indicator_file)
{
	int i =0;
	...
	for (auto target : target_prices)
	{
		...
		cout << neuron.weight_prices << "  " << neuron.weight_0 << "  "<< neuron.weight_1 << "  " << neuron.weight_2 << "  " << neuron.weight_3 << "  "<< neuron.weight_4 << "  "
				<< neuron.output<<endl;
		i++;
	}
	test_Neurons(neuron);
...
int main()
{
	...
	trainCells(testNeurons, target_prices, indicator_file);
